...
{
    // For used with any statement that spans multiple lines, like Sub/Function, If..then...else, Loop & etc
    public class VBBaseBlockStatement : VBBaseStatement 
    {
        internal VBBaseBlockStatement(VB6CodeModelFactoryContext factoryContext) : base(factoryContext) { }
        ...
        internal override void AfterVisitChild()
        {
            ...
            for (int i = RootModule.LastScanTokenIndex; i <= ParserContext.Stop.TokenIndex; i++)
            {
                ...
                {
                    case VisualBasic6Parser.NEWLINE:
                        this.AddCodeModel(new VBNewLine(token)); //ParentCodeBlock.AddCodeModel(new VBNewLine(token));
                        break;
                    case VisualBasic6Parser.COMMENT:
                        var isOnNewLine = true;
                        if (token.TokenIndex > 0)
                            isOnNewLine = (RootModule.VB6CommonTokenStream.Get(token.TokenIndex - 1).Type == VisualBasic6Parser.NEWLINE);
                        this.AddCodeModel(new VBComment(token, isOnNewLine)); //ParentCodeBlock.AddCodeModel(new VBComment(token, false));
                        break;
                    default:
                        // ignore the rest
                        break;
